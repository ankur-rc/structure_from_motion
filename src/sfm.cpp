...
private:
    ...
    const double FOCAL_LENGTH_X = 4308 / IMAGE_DOWNSAMPLE; // focal length in pixels, after downsampling, guess from jpeg EXIF data
    const double FOCAL_LENGTH_Y = 4308 / IMAGE_DOWNSAMPLE; // focal length in pixels, after downsampling, guess from jpeg EXIF data
...
private:
    ...
    void visualise_pointcloud(std::string name = "Point Cloud")
    {
...
int main(int argc, char *argv[])
{
    ...
    int downsample = 4;
    double fx, fy;
    fx = (4368 * 24 / 36) / downsample;
    fy = (2912 * 24 / 24) / downsample;
